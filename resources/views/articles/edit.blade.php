                ...
                <div class="p-6 text-gray-900">
                    <form action="{{ route('articles.update', $article->id) }}" method="POST">
                        ...
                        <div>
                            ...
                            <div class="my-3">
                                <textarea name="text" placeholder="Content" id="text" class="border-gray-300 shadow-sm w-1/2 rounded-lg" cols="30" rows="10">{{ old('text', $article->text) }}</textarea>
                            </div>

                            <label for="author" class="text-lg font-medium">Author</label>
                            <div class="my-3">
                                <input value="{{ old('author', $article->author) }}" id="author" name="author" placeholder="Enter Author" type="text" class="border-gray-300 shadow-sm w-1/2 rounded-lg">
                                @error('author')
                                    <p class="text-red-400 font-medium">{{ $message }}</p>
                                @enderror
                            </div>
